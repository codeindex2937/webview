...
public:
    ...
    ~StreamLoader();
    void release();
    IStream* get(const std::string& path);
};

StreamLoader::StreamLoader(): buffer(nullptr), stream(nullptr) {}
StreamLoader::~StreamLoader() {release();}
...
IStream* StreamLoader::get(const std::string& path) {
    release();
    ...
    DWORD file_size = file.tellg();
    file.seekg(0, std::ios::beg);

    // Allocate a memory buffer to hold the file data
    buffer = new BYTE[file_size];

    // Read the file data into the memory buffer
    file.read(reinterpret_cast<char*>(buffer), file_size);

    // Create a memory-based stream from the file data
    stream = SHCreateMemStream(buffer, file_size);
    if (stream == nullptr) {
        release();
        return nullptr;
    }
...
public:
  ...
  //   auto stream = loader.get("test.jpg");
  //   if (stream == nullptr) {
  //       // Handle error
  //       return 0x80004005L;
  //   }
  ...
  //   m_env->CreateWebResourceResponse(stream, 200, L"OK", L"", &response);

  //   ICoreWebView2HttpResponseHeaders *headers;
  //   if (0 == response->get_Headers(&headers)) {
...
public:
  ...
  void map_dir(const wchar_t* hostname, const wchar_t* folderPath) {
    ((ICoreWebView2_3*)m_webview)->SetVirtualHostNameToFolderMapping(hostname, folderPath, COREWEBVIEW2_HOST_RESOURCE_ACCESS_KIND_ALLOW);
  }
  ...
  bool embed(HWND wnd, bool debug, msg_cb_t cb) {
    ...
    m_com_handler = new webview2_com_handler(
        ...
        [&](ICoreWebView2Controller *controller, ICoreWebView2 *webview) {
          ...
          // EventRegistrationToken token;
          // auto rc = m_webview->add_WebResourceRequested(m_com_handler, &token);
...
public:
  ...
  void unbind(const std::string &name) {
    ...
    if (found != bindings.end()) {
      auto js = "delete window.eel['" + name + "'];";
      init(js);
      eval(js);
...
WEBVIEW_API void webview_map_dir(webview_t w, const wchar_t* hostname, const wchar_t* folderPath) {
  static_cast<webview::webview *>(w)->map_dir(hostname, folderPath);
}
