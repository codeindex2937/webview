...
static void CgoWebViewBind(webview_t w, const char *name, uintptr_t index) {
...
type WebView interface {
	...
	MapDir(hostname, folderPath string)
}
...
func NewWindow(debug bool, window unsafe.Pointer) WebView {
	...
	return &webview{w: C.webview_t(res)}
}
...
func (w *webview) MapDir(hostname, folderPath string) {
	hostnameUtf16, _ := windows.UTF16PtrFromString(hostname)
	absPath, _ := filepath.Abs(folderPath)
	absPathUtf16, _ := windows.UTF16PtrFromString(absPath)
	C.webview_map_dir(w.w, (*C.wchar_t)(hostnameUtf16), (*C.wchar_t)(absPathUtf16))
}
...
func _webviewBindingGoCallback(w C.webview_t, id *C.char, req *C.char, index uintptr) {
	...
	status, result := 0, ""
	if res, err := cb.fn(C.GoString(id), C.GoString(req)); err != nil {
		status = -1
		result = jsString(err.Error())
		log.Printf("error (%v): %v", cb.name, err)
	} else if b, err := json.Marshal(res); err != nil {
		status = -1
		result = jsString(err.Error())
		log.Printf("invalid result (%v): %v", cb.name, err)
	} else {
		status = 0
		result = string(b)
	}
...
func (w *webview) Bind(name string, f interface{}) error {
	...
	m.Lock()
	bindings[index] = &callback{name, binding}
	m.Unlock()
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	C.CgoWebViewBind(w.w, cname, C.ulonglong(index))
	return nil
}
